...
* **A Menu Bar for Cove (macOS):** Cove now has an official menu bar companion, created by [Robby McCullough](https://github.com/RobbyMcCullough/cove-menubar) and folded into the project with his blessing. The icon shows the stack's health at a glance — full color when Caddy, MariaDB, and Mailpit are all running, light grayscale when only some are, dark grayscale when everything's stopped — and the menu can start or stop Cove, refresh status, and jump straight to the Dashboard, Adminer, Mailpit, or the logs folder, with an optional launch-at-login toggle. It's a dependency-free native app that stays true to Cove's single-file philosophy: the Objective-C source ships embedded inside `cove.sh` itself, and `cove menubar enable` compiles it locally in seconds (using the Command Line Tools every Homebrew machine already has), installs it to `~/Applications`, and launches it — no separate download, no notarization dance. Strictly opt-in: `cove upgrade` never installs it on its own, but once enabled it keeps the app current automatically, and `cove menubar disable` removes it cleanly. If you were running Robby's original standalone build, enabling the official one simply takes its place. Beyond service control, the menu carries a **Sites submenu** listing every site with one-click open — hold Option on a WordPress site to generate a one-time admin login instead — plus quick links to the Dashboard, Adminer, Mailpit, the logs folder, and the Sites folder. The app also watches the stack for you: if a service dies out from under an otherwise-running stack it posts a macOS notification (deliberately staying quiet when everything stops at once, which usually just means you ran `cove disable`), and it checks GitHub once a day for new Cove releases, offering an "Update Cove" menu item that runs `cove upgrade` in a real Terminal window. Status polling is battery-considerate — a 20-second timer with generous tolerance, refreshed instantly whenever the menu opens — and it reads the new machine-readable status output, so PHP-FPM pools for version-pinned sites are monitored right alongside Caddy, MariaDB, and Mailpit. Larger installs get a "Recent" section at the top of the Sites submenu (top five by the same modified signal the dashboard sorts on), "New Site…" creates a WordPress site from a name prompt and opens it when ready, and a "Reload Caddy" item covers the regenerate-and-reload you'd otherwise open a terminal for after editing a directive.
...
* **Domain and Proxy Values Are Now Validated Before They Reach the Caddyfile and /etc/hosts:** `cove mappings add` and `cove proxy add` accepted their domain (and the proxy target) with only an is-it-empty check, then wrote the raw value straight into the generated Caddyfile and — for mappings — into a root-written `/etc/hosts` line. A value carrying Caddy syntax could inject an arbitrary server block (binding extra ports, proxying elsewhere, serving an attacker-chosen docroot), and because the `/etc/hosts` block was assembled with `echo -e`, a literal `\n` in a mapping could expand into an additional root-written host entry (local DNS spoofing). Both commands now reject anything that isn't a bare hostname via a shared `validate_hostname` helper, and the `/etc/hosts` writer uses `printf '%s\n'` so a backslash sequence can never expand — the same discipline the Tailscale proxy path already applied to its target. The same check now also covers the Tailscale hostname itself (`cove tailscale enable`), which is likewise interpolated into Caddyfile server-block labels — a stray space or brace in a pasted hostname used to take every site down at the next reload. The allowed charset is letters, numbers, dots, hyphens, and underscores, plus an optional leading `*.` wildcard label (proxy targets also allow a port colon), so dev hostnames like `my_site.test` and wildcard mappings that worked before keep working. Surfaced by a security audit of the CLI.
* **Menu Bar App Can No Longer Wedge on a Stuck Command:** Every `cove` call the menu bar app makes now runs under a timeout with a fire-once completion guard, so a subprocess that hangs (a wp-cli or PHP child during site creation that inherits the pipe and outlives `cove`) can no longer leave the app stuck on "Starting…"/"Creating…" with every control disabled until relaunch — the watchdog reclaims it (20s for status, 45s for login, two minutes for start/stop, four for a full site create). Two related fixes rode along: an unreadable status response no longer clears its own error and shows "Checking Cove status…" forever, and the one-time login now picks the URL that actually points at the target site's `wp-login` endpoint (a preceding wp-cli/PHP notice URL used to win and burn the token on the wrong page), with a wider token charset so it can't truncate mid-URL. Surfaced by a security/robustness audit of the app.
* **Menu Bar Crash Alerts Wait for a Second Opinion:** A "stopped unexpectedly" notification now requires the service to be seen down on two consecutive status checks before it fires, and starting an upgrade from the menu counts as a user action. Previously a status poll landing mid-`cove upgrade` — or during a terminal-run `cove enable` restart — could catch a service between stop and start and alert the user about a crash in an operation they launched themselves. A service that genuinely dies still notifies, just one poll cycle later.
* **Menu Bar Login Is Single-Flight:** Generating a one-time admin login takes wp-cli a few seconds, and the menu gave no feedback while it worked — an impatient second click ran a parallel `cove login`, which regenerated the one-time token and invalidated the first URL moments before its tab opened, landing on an "invalid token" page next to a working one. The menu now shows a "Logging in…" busy state for the duration and ignores further clicks until the first attempt finishes.
* **Homebrew Prompt Works Under `curl | bash`:** On a fresh Mac without Homebrew, the installer's "install it now?" confirmation used a bare `read` — but under the documented `curl https://cove.run/install-cove.sh | bash`, stdin is the script itself, so the `read` swallowed a byte of the script instead of the keypress and the install always aborted as "declined." The prompt now reads from `/dev/tty` behind the same guard the end-of-install handoff already used, so first-time macOS installs complete. Surfaced by the same audit.
